...
flowchart LR
    subgraph sources["Data sources (pick one)"]
        NV["NV Play / PCS Pro<br/>scorer's laptop writes a<br/>JSON frame every ball<br/>(optionally separate hardware,<br/>mirrored by nvplay_bridge.py<br/>or scorer_agent.py)"]
...
- **NV Play can run on hardware separate from the server, two ways.** `nvplay_bridge.py`
  serves the scorer's file over HTTP, token-gated, for machines that aren't on the same
  network (reached over Tailscale); `scorer_agent.py` is the simpler option for two laptops
  already on the same club wifi — found automatically via a UDP broadcast, no address or
  token to type. Either way the server mirrors the feed locally, becoming an ordinary local
  file (or, for the agent, an equivalent in-memory frame) before `parse_pcs_json()` — or
  anything else — ever sees it; `read_score_source()` is the one dispatch point every
  /live-adjacent call site goes through. Built so the streaming machine doesn't have to
  carry a scorer's VM's CPU/heat load alongside OBS.
